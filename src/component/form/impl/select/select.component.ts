...
@Component({
  ...
  styleUrls: ['./select.component.scss', './../../../../../assets/core/scss/style.scss'],
  providers: buildProviders(SelectComponent),
  viewProviders: VIEW_PROVIDERS,
  encapsulation: ViewEncapsulation.None
})
export class SelectComponent extends ReactiveInput implements OnInit {
  ...
  get select2Data(): Select2OptionData[] {
    let options = this.displayConfig.select2Config?.data || [];
    return options.filter(option => option.text.toLowerCase().includes(this.searchFilter.toLowerCase()))
  }
  ...
  ngOnInit(): void {
    ...
    this.select2ContainerSelector =  `#${this.displayConfig.formControlName} .select-selection`;
    this.setVisibleDownwards();
  }
